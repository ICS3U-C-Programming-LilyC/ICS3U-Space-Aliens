...
def menu_scene():
  ...
  text1 = stage.Text(width=29,
                     height=12,
                     font=None,
                     palette=constants.RED_PALLETTE,
                     buffer=None)
...
def game_scene():
  ...
  # This function takes an alien from off the screen and moves onto the screen.
  def show_alien():
    # Using a for loop to go through all of our aliens.
    for alien_number in range(len(aliens)):
      # Checking if the alien is off the screen.
      if aliens[alien_number].x < 0:
        # If it is off the screen then move it to a random spot on the screen.
        aliens[alien_number].move(
            random.randint(0 + constants.SPRITE_SIZE,
                           constants.SCREEN_X - constants.SPRITE_SIZE),
            constants.constants.OFF_TOP_SCREEN)
    ...
    # Creating an alien list.
    aliens = []
    # For loop to look through all 5 of our aliens
    for alien_number in range(constants.TOTAL_NUMBER_OF_ALIENS):
      # Creating 5 seperate aliens, using the 9th image in the image bank.
      # Setting there coordinates to be off the screen.
      a_single_alien = stage.Sprite(image_bank_sprites, 9,
                                    constants.OFF_SCREEN_X,
                                    constants.OFF_SCREEN_Y)
      # Adding each alien to the list.
      aliens.append(a_single_alien)

      # Placing 1 alien on the screen using the show_alien function.
      show_alien()
          ...
          if lasers[laser_number].x > 0:
            ...
            if lasers[laser_number].y < constants.OFF_TOP_SCREEN:
              ...
              lasers[laser_number].move(constants.OFF_SCREEN_X,
                                        constants.OFF_SCREEN_Y)
      # Using a for loop to loop through all of our aliens.
      for alien_number in range(len(aliens)):
        #  Using an if statement to check if the alien is on the screen.
        if aliens[alien_number].x > 0:
          # Moving the alien 1 pixel down the screen by increasing its y coordinate by 1.
          aliens[alien_number].move(aliens[alien_number].x,
                                    aliens[alien_number].y,
                                    constants.ALIEN_SPEED)
          # Using an if statement to check if an alien has gone off the cscreen.
          if aliens[alien_number].y > constants.SCREEN_Y:
            # Take the alien and move it back to the holding area.
            aliens[alien_number].move(constants.OFF_SCREEN_X,
                                      constants.OFF_SCREEN_Y)
            # Then taking another alien which will repeat the process.
            show_alien()
